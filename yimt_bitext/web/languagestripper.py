...
import urllib
import re
from urllib.parse import urlparse, urlsplit, urlunsplit,parse_qsl,ParseResult
...
class LanguageStripper(object):
    ...
    def __init__(self, languages=None, strip_query_variables=False):
        ...
        for code in ['zh-cn', 'zh-hk', 'zh-mo', 'zh-sg', 'zh-tw']:
            self.code_to_language[code] = 'zh'
        for code in ['ms-bn',  'ms-my']:
            self.code_to_language[code] = 'ms'
    ...
    def strip_netloc(self, netloc):
        components = []
        for c in netloc.split('.'):
            if not c:
                components.append(c)
                continue
            stripped = self.re_strip.sub('', c)
    ...
    def strip_uri(self, uri, expected_language=None,
        ...
        stripped_uri = ParseResult(scheme='http',
                                            netloc=self.strip_netloc(parsed_uri.netloc),
                                            path=stripped_path,
                                            params='',
